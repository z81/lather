...
class Task {
    ...
    tapChain(fn) {
        this.runtime.then({
            fn: (val) => (0, callHandled_1.callHandled)(() => fn(val).provide(this.env).runUnsafe(), [], () => val, (err) => {
                throw err;
            }),
            name: "tapChain",
            branch: taskRuntime_1.TaskBranches.Success,
        ...
        this.runtime.then({
            fn: (val) => {
                ...
                this.runtime.addHook(taskRuntime_1.Triggers.End, () => {
                    if (this.runtime.position >= this.runtime.callbacks.length &&
                        fn(this.runtime.branches[this.runtime.branchId])) {
    ...
    sequenceGen(fn) {
        ...
        this.runtime.then({
            fn: (val) => {
                ...
                let pos = this.runtime.position;
                this.runtime.addHook(taskRuntime_1.Triggers.End, (branchId) => {
                    // @ts-expect-error
    ...
    flat() {
        this.castThis().runtime.then({
            ...
            fn: (value) => {
                const pos = this.runtime.position;
                this.runtime.addHook(taskRuntime_1.Triggers.End, () => {
                    if (this.runtime.branchId === taskRuntime_1.TaskBranches.Success &&
                        value.length > 0) {
                        this.runtime.branches[taskRuntime_1.TaskBranches.Success] = value.pop();
    ...
    static sequenceFromIterable(itererable) {
        ...
        return Task.empty
            .sequenceGen(() => (iter[Symbol.iterator] || iter[Symbol.asyncIterator]).call(iter))
            .castThis();
    }
    ...
    structPar(struct) {
        this.runtime.then({
            name: "structPar",
            fn: async () => {
                return Object.fromEntries(await Promise.all(Object.entries(struct).map(async ([name, task]) => [
                    name,
